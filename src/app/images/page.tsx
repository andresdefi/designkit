...
import { imageStyles, IMAGE_GROUPS, IMAGE_META } from "@/data/images";
...
export default function ImagesPage() {
  ...
  const selectedId = selections.images;
  const colors = usePreviewColors();

  function handleSelect(id: string) {
    if (selectedId === id) deselect("images");
    else select("images", id);
  }
  ...
  return (
    <>
      <TopBar
        ...
        description="Photo framing, overlays, shapes, and aspect ratio styles"
        itemCount={imageStyles.length}
      />
      <CatalogGrid columns={3}>
        {IMAGE_GROUPS.map((group) => {
          const groupItems = imageStyles.filter(
            (item) => IMAGE_META.find((m) => m.id === item.id)?.group === group
          );
...
function ImagePreviewCard({
  ...
  item: (typeof imageStyles)[number];
  isSelected: boolean;
  colors: ColorMode;
  onSelect: (id: string) => void;
}) {
  const { css, overlayCss, hasOverlay, hasCaption, variant } = item.data;
  const containerStyle = resolveCSS(css, colors);
  const overlayStyle = overlayCss ? resolveCSS(overlayCss, colors) : undefined;
  ...
  return (
    ...
    >
      ...
      <div className="p-4">
        ...
        >
          <div style={{ ...containerStyle, maxWidth: variant === "circle" ? "80px" : "100%", position: "relative" }}>
            <div
              style={{
                width: "100%",
                aspectRatio: item.data.aspectRatio ?? "16/9",
                maxHeight: "80px",
                background: `linear-gradient(135deg, ${hexToRgba(colors.primary, 0.25)}, ${hexToRgba(colors.accent, 0.25)})`,
            ...
            {hasOverlay && overlayStyle && (
              <div style={overlayStyle}>
                {hasCaption && (
                  <>
                    <div style={{ fontSize: "10px", fontWeight: 600 }}>Image Title</div>
                    <div style={{ fontSize: "8px", opacity: 0.7 }}>Subtitle text</div>
                  </>
                )}
              </div>
            )}
