...
import { dividerStyles, DIVIDER_GROUPS, DIVIDER_META } from "@/data/dividers";
...
export default function DividersPage() {
  ...
  const selectedId = selections.dividers;
  const colors = usePreviewColors();

  function handleSelect(id: string) {
    if (selectedId === id) deselect("dividers");
    else select("dividers", id);
  }
  ...
  return (
    <>
      <TopBar
        title="Dividers"
        description="Separators, visual breaks, and decorative lines"
        itemCount={dividerStyles.length}
      />
      <CatalogGrid columns={3}>
        {DIVIDER_GROUPS.map((group) => {
          const groupItems = dividerStyles.filter(
            (item) => DIVIDER_META.find((m) => m.id === item.id)?.group === group
          );
...
function DividerPreviewCard({
  item,
  isSelected,
  colors,
  onSelect,
}: {
  item: (typeof dividerStyles)[number];
  isSelected: boolean;
  colors: ColorMode;
  onSelect: (id: string) => void;
}) {
  const { css, hasLabel, hasIcon, variant } = item.data;
    ...
    >
      ...
      <div className="p-4">
        <div
          ...
          style={{ backgroundColor: colors.background, border: `1px solid ${colors.border}`, minHeight: "60px" }}
        >
          <span className="text-[9px]" style={{ color: colors.textMuted }}>Content above</span>
          {hasLabel ? (
            <div style={style}>
              <span style={{ flex: 1, height: "1px", backgroundColor: colors.border }} />
              <span style={{ fontSize: "11px" }}>or continue with</span>
              <span style={{ flex: 1, height: "1px", backgroundColor: colors.border }} />
            </div>
          ) : hasIcon ? (
            <div style={style}>
          ...
          <span className="text-[9px]" style={{ color: colors.textMuted }}>Content below</span>
        </div>
      </div>
